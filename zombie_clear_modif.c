...
/**
 * Create multiple child processes - only child 2 and 4
 */
void create_children(void) {
    char msg[256];
    int child_ids[] = {1, 2, 3, 4, 5}; // Only create child 2 and 4
    ...
    for (int i = 0; i < NUM_CHILDREN; i++) {
        ...
        if (pid == 0) {
            ...
            snprintf(msg, sizeof(msg), "Created child %d with PID: %d", child_ids[i], pid);
            log_message(msg);
        } else {
            // Fork failed
            snprintf(msg, sizeof(msg), "ERROR: Failed to create child %d: %s", 
                    child_ids[i], strerror(errno));
            log_message(msg);
        }
    }
